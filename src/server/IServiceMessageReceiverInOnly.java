/**
 * IServiceMessageReceiverInOnly.java
 *
...
/**
 *  IServiceMessageReceiverInOnly message receiver
 */
public class IServiceMessageReceiverInOnly extends org.apache.axis2.receivers.AbstractMessageReceiver {
        ...
        try {
            ...
            if ((op.getName() != null) &&
                ...
                if ("addItem".equals(methodName)) {
                    //doc style
                    soap.AddItem wrappedParam = (soap.AddItem) fromOM(msgContext.getEnvelope()
                                                                                .getBody()
                                                                                .getFirstElement(),
                            soap.AddItem.class);

                    skel.addItem(wrappedParam);

                    envelope = getSOAPFactory(msgContext).getDefaultEnvelope();
                } else if ("deleteItem".equals(methodName)) {
                    //doc style
                    soap.DeleteItem wrappedParam = (soap.DeleteItem) fromOM(msgContext.getEnvelope()
                                                                                      .getBody()
                                                                                      .getFirstElement(),
                            soap.DeleteItem.class);

                    skel.deleteItem(wrappedParam);

                    envelope = getSOAPFactory(msgContext).getDefaultEnvelope();
                } else if ("updateItem".equals(methodName)) {
                    //doc style
                    soap.UpdateItem wrappedParam = (soap.UpdateItem) fromOM(msgContext.getEnvelope()
                                                                                      .getBody()
                                                                                      .getFirstElement(),
                            soap.UpdateItem.class);

                    skel.updateItem(wrappedParam);
